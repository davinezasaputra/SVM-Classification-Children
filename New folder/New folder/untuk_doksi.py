...
# pemilihan fitur
df = df.dropna(subset=['bb', 'tb_koreksi', 'usia', 'jk'])
X = df[['bb', 'tb_koreksi', 'usia', 'jk']]
y = df['BB/TB']
...
axes[0].set_title('Distribusi Data Training (Sebelum SMOTE)', fontsize=14, fontweight='bold')
...
axes[1].set_title('Distribusi Data Training (Setelah SMOTE)', fontsize=14, fontweight='bold')
...
#     y_min, y_max = X['tb_terkalibrasi'].min() - 5, X['tb_terkalibrasi'].max() + 5
#     xx, yy = np.meshgrid(np.arange(x_min, x_max, 0.2),
#                          np.arange(y_min, y_max, 1.0))

#     grid_df = pd.DataFrame({
#         'bb': xx.ravel(),
#         'tb_terkalibrasi': yy.ravel(),
#         'jk_encoded': np.full(xx.ravel().shape, gender_val)
#     })
#     Z = pipeline_bbtb.predict(grid_df)
#     Z = Z.reshape(xx.shape)
#     # map
#     plt.contourf(xx, yy, Z, alpha=0.35, cmap=custom_cmap)
#     idx = X_test['jk_encoded'] == gender_val
#     plt.scatter(X_test[idx]['bb'], X_test[idx]['tb_terkalibrasi'], 
